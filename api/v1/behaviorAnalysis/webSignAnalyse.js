...
const webSignAnalyse = function(req, res){
    ...
    if(type === 'load'){
        //获取埋点配置，修改页面访问统计
        webPageviewFilterRecord(req,res)
    }else if(type === 'catch'){
        //上报用户埋点行为，
        filterWebObserverRecord(res,params)
    }else{
...
// 获取页面配置时过滤修改操作
function webPageviewFilterRecord(req, res) {
    var params = req.body;
    params.areaInfo = {
        newIp: req.ip,
        oldIp: null,
...
//上报埋点操作数据
function filterWebObserverRecord(res, params) {
    ...
    sqlModel.findOne({
        ...
        if (result) {
            ...
            ajaxFailResult(res, 'not find currentUser record')
        }
    })
}
